...
## 1.1. 머스테치
머스테치는 현존하는 대부분의 언어를 지원하는 가장 심플한 템플릿 엔진이다.(JSP+JSTL,Tiles,SiteMesh,REACT 같은)   
...
var main = {
    ...
    save : function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/api/v1/posts',
            dataType: 'json',//결과값을 받는 데이터형식 text-String, json-Map<String,Object>
            contentType: 'application/json; charset=utf-8',
            data: JSON.stringify(data)//처리할 값을 보내는 데이터형식 -> json을 텍스트형식으로 변환해서
        }).done(function(){
            alert('글이 등록되었습니다.');
...
main.init()
```
위에서 왜 ```var main = { 함수이름 : function(){}}```이런식으로 만들었을까?     
...
즉, PostsRepository 결과로 넘어온 List에 저장된 각각의 Posts 를 PostsListResponseDto객체를 만들고 다시 리스트에 넣는다.   
