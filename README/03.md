...
그래서 이러한 문제의 해결책으로 Java Persistence API(이하 "JPA"로 표기)라는 자바 표준 ORM 기술을 사용해보자    
...
## 1.1. SpringData JPA 
JPA(Java Persistence API)는 인터페이스로 자바 표준 명세서이다.  
...
점점 트래픽이 많아져 관계형 데이터베이스로는 도저히 감당이 안될 때 no-sql로 교체를 할 수도 있다.(Mongo DB   
...
h2
  ...
  * 비슷한 임베디드형 DB로 자바DB인 hsql 이 사용된다.  
``` 

### 1.3.2. 폴더(패키지) 생성    
**java 폴더 -> 디폴트 패키지에 domain 폴더 생성** 
   
domain은 게시글, 댓글, 회원, 정산, 결제 등 소프트웨어에 대한 요구사항 필드 영역이라 생각하자  
기존 객체,DAO(@Repository),쿼리 xml 구조 대신 @Entity 클래스만으로 해결할 수 있다는 차이점에서 나온 용어이다.   
...
JPA를 사용한다면 DB에 DDL(DCL)작업을 할 경우 실제 쿼리를 날리기보다는, 이 Entity 클래스의 수정을 통해 작업한다.       
...
* Web Layer
     * 흔히 사용하는 @Controller 와 JSP,JSTL/Tiles,Mustache 등의 뷰 템플릿 영역입니다.  
     * 이외에도 AOP(필터, 인터셉터, 컨트롤러 어드바이스)등 외부 요청과 응답에 대한 전반적인 영역을 이야기합니다.  
...
이 5가지 영역에서 비지니스 처리를 담당하는 곳은 바로 Domain이다.         
이전에는 Service 방식에서 처리를 했고 이를 트랜잭션 스크립트라 불렀는데       
도메인이라는 개념이 생기고 나서 이러한 방식이 바뀌게 되었다.    

도메인 모델(객체)에서 처리할 경우 다음과 같은 코드가 될 수 있다.
...
### 1.4.2. 등록/수정/삭제 기능 만들기  작업순서(아래)
1. Posts 엔티티클래스와 JpaRepository 인터페이스를 생성한다.
2. PostsSaveRequestDto를 web.dto 패키지에 생성한다.(DTO는 엔티티와 서비스, 서비스와 컨트롤러, 컨트롤러와 jsp의 데이터전송객체로 사용)
3. PostsService를 service 패키지에 생성한다.
4. PostsApiController를 web 패키지에 생성한다.
...
그 이유는 Entity 클래스를 ```Request/Response``` 클래스로 사용하는 것은 매우 안좋기 때문이다.(스프링레거시에서는 아닙니다.^^)
...
스프링에서 Bean을 주입받는 방식은 2가지이다.
1. ```@Autowired|@Inject``` (필드)
2. 생성자

기존 스프링에서는 ```@Autowired (필드)```을 주로 사용하지만 이는 좋은 방법은 아니다. (자바8부터 @Inject 도 사용합니다.)
...
꼭 위 방식이 정답은 아님. @Inject 사용해도 위 1~4 문제가 되지 않습니다.
...
```
@Controller 테스트는```@WebMvcTest```를 사용하는데, 사용하지 않는 이유는 JPA 기능이 작동하지 않기 때문이다.    
그렇기에 URL호출 및 JPA 기능까지 한번에 테스트할 때는 ```@SpringBootTest```와 ```TestRestTemplate```을 사용하면 된다.    
...
단, SecurityConfig.java에서 .antMatchers("/api/v1/**").permitAll()로 JUnit테스트 때만 모두 허용한다.
...
앞으로 추가될 엔티티티들은 더이상 등록일/수정일로 고민할 필요가 없다.    
